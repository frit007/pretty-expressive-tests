...
def concatBenchmark (doc : Doc) (state : FormatState) (c : Config):  IO Info := do
...
def main (originalArgs : List String): IO Unit := do
  ...
  let (doc, state) := simpleFormattingContext (pp cfg.size) 4
  runBenchmark "concat" cfg (concatBenchmark doc state)
