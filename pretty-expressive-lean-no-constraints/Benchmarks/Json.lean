...
def jsonBenchmark (doc : Doc) (state : FormatState) (c : Config):  IO Info := do
...
def main (originalArgs : List String): IO Unit := do
  ...
  | some s => s
  | _ => "../data"
  let path := path ++ (jsonFile cfg.size)
  let content ← IO.FS.readFile (path)

  let (doc, state) := match Json.parse content with
  | Except.ok json =>
    simpleFormattingContext (pp json) 4
  | Except.error _ => simpleFormattingContext (return "no" <_> "json")

  runBenchmark "json" cfg (jsonBenchmark doc state)
